...
namespace RethinkDb.Test.ConnectionFactories
{
    ...
    public class ConnectionPoolingConnectionFactoryTests
    {
        ...
        public void SetUp()
        {
            ...
            realConnection1.RunAsync(Arg.Any<IQueryConverter>(), (ISingleObjectQuery<int>)null, Arg.Any<CancellationToken>()).Returns(
                y => { var x = new TaskCompletionSource<int>(); x.SetResult(1); return x.Task; }
            );

            var realConnection2 = Substitute.For<IConnection>();
            realConnection2.RunAsync(Arg.Any<IQueryConverter>(), (ISingleObjectQuery<int>)null, Arg.Any<CancellationToken>()).Returns(
